...
export class InquiryComponent implements OnInit {
	...
	public type:any = 1;
	public addInquiry = false;	
	public inquiryRemark : any = '';
	...
	addNewInquiry(){
		this.errors = {};
		if(this.inquiryRemark.length > 0){
			let request = this.apiService.addUpdateInquary();	
			request.data.type = '1';
			request.data.message = this.inquiryRemark;
			this.apiService.submitRequest(request).subscribe( (res:any) => {
	  			let response =  this.apiService.parseResponse(res); 
	  			if(response.status == 'success'){
	  			 	this.dataList();
			...
			this.errors.inquiryRemark = 'This field is required';
		}	
	}
